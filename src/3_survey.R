...
# Cutoff to exclude test responses from research team
cutoff_datetime <- ymd_hms("2023-12-21 14:00:00")

responses <- read_csv(here("data", 
                           "LTBF survey December 21, 2023_20.09.csv")) %>% 
  clean_names() %>% 
  slice(3:n()) %>% 
  # The below filter excludes a small number of responses 
  # that failed the attention check
  filter(x14 == "5 people who live far from court",
         x15 == "5 Black people") %>% 
  mutate(start_date = ymd_hms(start_date),
         ...
         political_party = coalesce(independent_q,
                                    political_aff)) %>% 
  # As noted above, this cutoff excludes test responses from research team
...
ggsave(here("output", "figure_3b.pdf"),
       party_plot,
       width = 6.5, height = 3)
...
ggsave(here("output", "figure_ec1a.pdf"),
       gender_plot,
       width = 6.5, height = 3)
